...
public class ComfyUIApiClient : IImageGenerator
{
	...
	private async Task<List<DataContent>> ExtractAndDownloadImagesAsync(
		string promptId,
		...
		CancellationToken cancellationToken)
	{
		...
		var contents = new List<DataContent>();
		...
		foreach (var outputNode in outputs.AsObject())
		{
			...
			if (images != null && images.Count > 0)
			{
				...
				var filename = imageInfo?["filename"]?.GetValue<string>() ?? throw new InvalidOperationException("No filename found for image");
